...
def get_doctors(db: Session = Depends(get_db)):
    return db.query(DoctorDB).filter(DoctorDB.is_active.is_(True)).all()


@app.get("/api/appointments", response_model=PaginatedAppointments, tags=["Appointments"])
def list_appointments(
    ...
    if date_str:
        try:
            ...
            query = query.filter(
                AppointmentDB.start_time >= start_of_day,
                AppointmentDB.start_time <= end_of_day,
            )
        except ValueError:
...
def book_appointment(appt: AppointmentCreate, db: Session = Depends(get_db)):
    ...
    if overlapping:
        raise HTTPException(
            status_code=409,
            detail=(
                "Conflict: Doctor is already booked between "
                f"{overlapping.start_time.strftime('%H:%M')} and {overlapping.end_time.strftime('%H:%M')}."
            ),
        )
